...
# environment
game = 'Hunt'
env = gym.make("StagHunt-"+game+"-v0", obs_type="coords", max_episode_steps=200, stag_follows=False, load_renderer=True, enable_multiagent=False, opponent_policy="random")

actor = torch.load("random.pt", weights_only=False)
actor.eval()
obs1, _ = env.reset()
env.render()
time.sleep(1)
